...
| 8. Photo Merge | HDR, Panorama and HDR Panorama merges into float DNGs (`docs/PhotoMerge.md`) | A 3×24 MP tripod HDR in under 10 s; results open in Latent, Lightroom and Apple Photos | **In progress.** DNG writer, linear-source rendering and the `latent:Merge` recipe done; Photo › Photo Merge › HDR… (⌃H, tripod only) with its dialog, background job and naming (§8.6), built against the `HDRMerging` contract while the HDR engine lands. Later: alignment, deghosting, panoramas |
