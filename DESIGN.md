...
| 8. Photo Merge | HDR, Panorama and HDR Panorama merges into float DNGs (`docs/PhotoMerge.md`) | A 3×24 MP tripod HDR in under 10 s; results open in Latent, Lightroom and Apple Photos | **In progress.** DNG writer, linear-source rendering and the `latent:Merge` recipe done; HDR complete (⌃H with its dialog, preview, Auto Align, Deghost and Auto Settings; ⌃⇧H without the dialog). Panorama: the geometry, the stitcher and the app's Panorama… (⌃M) dialog and job are in, against the `PanoramaMerging` contract — one row, Auto Crop as an undoable crop edit, and a panorama too big to edit made smaller with the user's agreement, never refused. Next: HDR Panorama (Phase 9, experimental), multi-row and 360° |
