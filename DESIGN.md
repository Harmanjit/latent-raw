...
**Exports and location.** With metadata on (the export sheet's default), exports carry the photo's own GPS position, artist and copyright along with the camera tags. The sheet's metadata toggle strips all of it. Export Open Image in the left panel has no toggle and always writes metadata.
...
4. **Metadata** (on by default in the export sheet; always on for Export Open Image). The photo's own EXIF, GPS, IPTC, ExifAux and XMP are read in the decoder service (§4a). Removed: storage tags (compression, CFA pattern, orientation, pixel size, colour space, subject area and location, focal-plane resolution, AF areas), makers' private blocks, the source's document and instance IDs, and the `crs`, `hdrgm` and `HDRGainMap` namespaces. Summary fields from LibRaw fill gaps. Latent's keywords are added to the file's own; its rating replaces the file's when above 0. Software is Latent, orientation 1, Exif ColorSpace 1 for sRGB and 0xFFFF otherwise. No embedded thumbnail. With metadata off, the file carries no camera, date, location, keywords or rating.
...
**Catalog writes.** Ratings, keywords, saved edits, history, snapshots, batch paste and opening a folder go through `Library.perform`, which counts running operations. In the grid, rating, flag and rotation apply to every selected image, each with its own row update and sidecar write; a failure on one image doesn't stop the rest. In Loupe, Compare and Develop they apply to the image shown only. Keywords apply to the lead image only.

**Quitting.** `applicationShouldTerminate` saves a pending edit at once, then waits up to 10 s for catalog writes to finish. If an export is running it first asks whether to stop after the image being written and quit, or keep exporting and not quit; stopping waits up to 60 s for that file, and a stopped export does not bring Finder forward. A wait that runs out is logged and quitting goes ahead.
...
| Exports leak location | **In place:** one toggle in the export sheet strips camera metadata, GPS, keywords and rating (§8.5). **Open:** metadata is on by default, and Export Open Image has no toggle (§4a) |
