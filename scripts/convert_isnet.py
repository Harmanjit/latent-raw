#!/usr/bin/env python3
"""
Convert IS-Net (dichotomous image segmentation: fine wiry detail) from the
...
write the manifest the model registry reads.
    ...
    python scripts/convert_isnet.py --weights ~/Downloads/isnet-general-use.pth
    python scripts/convert_isnet.py --weights … --verify-only   # re-check the package already in --out

The result is a folder (`./isnet/` by default) holding
`ISNet_general_use.mlpackage` and `isnet.model.json`, which the user adds
through Settings › AI › Models.

IS-Net (Qin et al. 2022, https://github.com/xuebinqin/DIS) is a U²-Net-style
encoder-decoder trained on DIS5K at 1024². The code is Apache-2.0 but the
DIS5K training data is research-only, so the manifest says
`commercialUse: false` and Settings shows "Research use only". The weights
are not on Hugging Face: the repository's README links
`isnet-general-use.pth` on Google Drive, so the script takes the downloaded
file with --weights and checks its SHA-256 against the value pinned here.
A pin that still reads `<pin me>` refuses to convert until someone has
vetted the upload and recorded it.
...
2. Loads ISNetDIS from the repository's own IS-Net/models/isnet.py with the
   state dict strictly, so any layout mismatch fails loudly.
3. Wraps the model so its input is a plain RGB image (0-255) and the
   x/255 - 0.5 normalisation the inference script applies (mean 0.5, std 1)
   happens inside the graph; the first side output is already a sigmoid
   ...
   left out on purpose: it would turn a photo with no clear subject into a
...
# The git commit of xuebinqin/DIS that was vetted, and the SHA-256 of
# isnet-general-use.pth as the README links it.
CODE_REVISION = PLACEHOLDER
WEIGHTS_NAME = "isnet-general-use.pth"
WEIGHTS_SHA256 = PLACEHOLDER
LICENCE_URL = "https://github.com/xuebinqin/DIS/blob/main/LICENSE.md"
MODEL_ID = "isnet"
PACKAGE_NAME = "ISNet_general_use"
INPUT_SIZE = 1024         # what the model was trained at and its inference script resizes to

parser = argparse.ArgumentParser(description=__doc__.split("\n\n")[0],
                                 formatter_class=argparse.RawDescriptionHelpFormatter)
parser.add_argument("--weights", help=f"path to {WEIGHTS_NAME}, downloaded from the link in the DIS README")
...
if CODE_REVISION == PLACEHOLDER or WEIGHTS_SHA256 == PLACEHOLDER:
    sys.exit(f"convert_isnet.py is not pinned yet: vet {CODE_URL} and {WEIGHTS_NAME}, then record the git commit "
             f"in CODE_REVISION and the file's SHA-256 in WEIGHTS_SHA256 before converting. Nothing was fetched.")
if not args.weights:
    parser.error(f"--weights is needed: {WEIGHTS_NAME} from the link in the DIS README")
...
# ------------------------------------------------------ 1. code and weights
code = checkout(CODE_URL, CODE_REVISION, "DIS")
...
sys.path.insert(0, os.path.join(code, "IS-Net"))
from models.isnet import ISNetDIS     # the repository's own definition, at the pinned commit
model = ISNetDIS()
state = torch.load(args.weights, map_location="cpu", weights_only=True)
state = {k[len("module."):] if k.startswith("module.") else k: v for k, v in state.items()}
model.load_state_dict(state, strict=True)
model.eval()
print(f"  ISNetDIS, {sum(p.numel() for p in model.parameters())/1e6:.1f} M parameters")
...
class Wrapped(torch.nn.Module):
    ...
    def forward(self, image):
        x = image / 255.0 - 0.5                # Inference.py: normalize(x/255, mean 0.5, std 1.0)
        side_outputs, _ = self.net(x)          # ([sigmoid(d1) … sigmoid(d6)], features)
        return side_outputs[0]                 # d1: the full-resolution prediction, (1, 1, H, W)
...
if not args.verify_only:
    ...
    licence = f"Apache-2.0 code ({CODE_URL}); DIS5K training data is research-only"
    mlmodel.short_description = "IS-Net (DIS) subject/background segmentation: RGB image in, soft foreground mask [0,1] out"
    ...
    mlmodel.user_defined_metadata["input"] = "image: RGB 0-255, x/255 - 0.5 normalisation is inside the graph"
...
manifest = latent_manifest.build_manifest(
    id=MODEL_ID, display_name="IS-Net", purpose="Subject masks with fine wiry detail (DIS); research use only",
    version=1, kind="subjectSegmentation", licence_name="Apache-2.0 code, DIS5K training data research-only",
    licence_url=LICENCE_URL, commercial_use=False,
    source_url=CODE_URL, input_size=args.size, packages=[row],
    output_activation="probabilities", refine="none",
    converter={"script": "scripts/convert_isnet.py", "sourceRevision": CODE_REVISION,
