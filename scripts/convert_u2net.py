#!/usr/bin/env python3
"""
Convert U²-Net (salient-object detection: one-shot subject masks) from the
published PyTorch weights into a Core ML package for Latent's MLKit, and
write the manifest the model registry reads.

    source ~/latent-ml/bin/activate
    python scripts/convert_u2net.py --weights ~/Downloads/u2net.pth            # the full model, ./u2net/
    python scripts/convert_u2net.py --small --weights ~/Downloads/u2netp.pth   # U²-Net-P, ./u2net-small/
    python scripts/convert_u2net.py --weights … --verify-only   # re-check the package already in --out

The result is a folder holding `U2Net.mlpackage` (or `U2NetP.mlpackage`)
and its `<id>.model.json`, which the user adds through Settings › AI ›
Models. Both run with guided-filter refinement in the app (`refine:
guided`): the network works at 320², so its edges are soft.

U²-Net (Qin et al. 2020, https://github.com/xuebinqin/U-2-Net, Apache-2.0)
is a two-level nested U-structure; U²-Net-P is the 4.7 MB variant with the
same shape and narrower blocks. The weights are not on Hugging Face: the
repository's README links `u2net.pth` and `u2netp.pth` on Google Drive, so
the script takes the downloaded file with --weights and checks its SHA-256
against the value pinned here. A pin that still reads `<pin me>` refuses to
convert until someone has vetted the upload and recorded it.
...
2. Loads U2NET or U2NETP from the repository's own model/u2net.py with the
   state dict strictly, so any layout mismatch fails loudly.
3. Wraps the model so its input is a plain RGB image (0-255) and the
   ImageNet normalisation happens inside the graph. The repository's data
   loader divides by the image's own maximum rather than 255; for a photo
   that reaches white the two agree, and a fixed divisor is what a Core ML
   image input can express. The fused side output is already a sigmoid
   inside the network, so the output is a [0, 1] soft mask at the input
   resolution; the per-image min-max stretch the inference script adds is
   left out, as it would turn a photo with no clear subject into a
   full-contrast mask of noise.
...
5. Writes the .mlpackage and metadata, then verifies it against PyTorch fp32
   on a test photo: max abs diff, IoU at 0.5, timing at CPU_AND_GPU and
   CPU_ONLY. IoU under 0.97 on the GPU path fails the run.
...
# The git commit of xuebinqin/U-2-Net that was vetted.
CODE_REVISION = PLACEHOLDER
LICENCE_URL = "https://github.com/xuebinqin/U-2-Net/blob/master/LICENSE"
INPUT_SIZE = 320          # what the models were trained at and their inference script resizes to

# Each variant is pinned to the SHA-256 of its weights file as the README
# links it. A variant with no pin refuses to convert.
VARIANTS = {
    "full": dict(
        cls="U2NET", weights_name="u2net.pth", weights_sha256=PLACEHOLDER,
        name="U2Net", id="u2net", display_name="U²-Net",
        purpose="Subject masks: the classic salient-object model, softer edges",
    ),
    "small": dict(
        cls="U2NETP", weights_name="u2netp.pth", weights_sha256=PLACEHOLDER,
        name="U2NetP", id="u2net-small", display_name="U²-Net Small",
        purpose="Subject masks in an instant, for previews; the roughest edges",
    ),
}

parser = argparse.ArgumentParser(description=__doc__.split("\n\n")[0],
                                 formatter_class=argparse.RawDescriptionHelpFormatter)
parser.add_argument("--small", action="store_true", help="convert U²-Net-P (u2netp.pth) instead of the full model")
parser.add_argument("--weights", help="path to u2net.pth or u2netp.pth, downloaded from the link in the U²-Net README")
parser.add_argument("--size", type=int, default=INPUT_SIZE, help="square input size in pixels")
parser.add_argument("--out", default=None, help="directory for the package and manifest (default ./<id>/)")
...
variant = VARIANTS["small" if args.small else "full"]
WEIGHTS_NAME, WEIGHTS_SHA256 = variant["weights_name"], variant["weights_sha256"]
if CODE_REVISION == PLACEHOLDER or WEIGHTS_SHA256 == PLACEHOLDER:
    sys.exit(f"convert_u2net.py is not pinned yet for {variant['id']}: vet {CODE_URL} and {WEIGHTS_NAME}, then "
             f"record the git commit in CODE_REVISION and the file's SHA-256 in VARIANTS before converting. "
             f"Nothing was fetched.")
if not args.weights:
    parser.error(f"--weights is needed: {WEIGHTS_NAME} from the link in the U²-Net README")
out_dir = args.out or variant["id"]
...
# ------------------------------------------------------ 1. code and weights
code = checkout(CODE_URL, CODE_REVISION, "U-2-Net")
...
import model as u2net_module          # the repository's own model/u2net.py, at the pinned commit
model = getattr(u2net_module, variant["cls"])(3, 1)
state = torch.load(args.weights, map_location="cpu", weights_only=True)
state = {k[len("module."):] if k.startswith("module.") else k: v for k, v in state.items()}
model.load_state_dict(state, strict=True)
model.eval()
print(f"  {variant['cls']}, {sum(p.numel() for p in model.parameters())/1e6:.1f} M parameters")
...
class Wrapped(torch.nn.Module):
    ...
    def forward(self, image):
        ...
        return self.net(x)[0]                  # d0, the fused side output, sigmoid'ed inside: (1, 1, H, W)
...
if not args.skip_verify:
    ...
    assert 0 <= reference.min() and reference.max() <= 1, "the mask is not in [0, 1]; wrong output picked?"
...
if not args.verify_only:
    ...
    mlmodel.short_description = f"{variant['cls']} salient-object segmentation: RGB image in, soft foreground mask [0,1] out"
...
manifest = latent_manifest.build_manifest(
    id=variant["id"], display_name=variant["display_name"], purpose=variant["purpose"],
    ...
    converter={"script": "scripts/convert_u2net.py" + (" --small" if args.small else ""),
               "sourceRevision": CODE_REVISION,
