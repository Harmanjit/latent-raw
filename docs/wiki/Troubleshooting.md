...
The sentence under the list says which check failed. The folder must hold exactly one `<id>.model.json` manifest and the Core ML packages it names, and nothing else; each package's contents must match the checksum in the manifest, which is why a model must be converted by the script in `scripts/` rather than assembled by hand or edited afterwards. "The manifest carries no checksums" means the folder holds a catalogue row copied from the app, not a converted model. Choosing a `.mlpackage` on its own works only when its manifest sits beside it or inside it at the package's root; "No .model.json manifest was found beside … or inside it" means neither, so choose the folder the conversion script wrote instead. "names more packages than … alone" means a manifest inside a package lists packages that are not there, which only a folder can hold. An archive that unpacks to far more than a model needs, or holds far more entries, is refused before anything is written. "Core ML could not compile" names a package this macOS cannot build; the model page may say which version it needs. A refused import leaves nothing behind.
