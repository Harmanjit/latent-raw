...
**Show Loupe on Second Display** (View menu, with two or more displays) fills another display with the image at fit and its caption, clear of that display's menu bar and Dock. It is for looking only: zoom, pan and tools stay in the main window. It shows what Develop, Loupe or Compare's candidate shows, before/after and the mask overlay included; in Survey, the outlined pane; from the grid it shows the selected image. With HDR display on, the image is rendered for the brighter of the two screens, so an ordinary main screen beside an XDR display shows the HDR render with its highlights fitted to it, as an XDR screen at low brightness does. It closes when its display is unplugged or the main window closes.
...
Ratings, flags, keywords, edits, snapshots, history and the thumbnail go with each image. If the destination already has a file of that name, or a sidecar left behind by one, the image gets a number ("DSC_0107 2.NEF") and its old name is kept in the sidecar as its original file name. A folder without a catalog gets a `_latent` folder only when an image brings a sidecar. A subfolder inside the open catalog that the catalog doesn't include (one you chose to keep separate, still to be asked about, or just made with New Folder in the panel) becomes a catalog of its own.

Progress shows in the status bar with a stop button. Stopping, or quitting, finishes the image under way, so no image is left half-moved. ⌘Z moves images back; undoing a copy puts the copy and its sidecar in the Trash. Nothing is ever overwritten or permanently deleted, apart from thumbnails, which are made again, a moved image's old sidecar, whose contents are already in the new one, and, for a move to another disk, the original once its copy is complete.
