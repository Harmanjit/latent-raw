...
Loupe and Develop show a filmstrip of the grid's images along the bottom; the film button in the status bar hides it.

## Rate and flag

0–5 for stars, P to pick, X to reject, U to clear. ⌘[ and ⌘] rotate. In the Library grid these act on every selected image; in Loupe, Compare and Develop, on the image shown. Keywords, typed in the left panel, apply only to the image named there (with several selected, the one clicked last). All of these are written to the sidecar immediately and survive a relaunch, a catalog rebuild, or a move of the folder to another Mac.
