...
- **On-image tools.** The crop rectangle reads its size, aspect and angle, and has a Reset crop action. Spot removal patches read how many there are and which is selected; adjust to step the selection through them, and use the Delete selected patch action. Red-eye spots work the same way, with a Delete selected spot action, and the panel's Auto button adds spots without a pointer; what it found is announced. Sensor Dust's rings read as "37 dust spots, spot 3 selected", adjust to step through them, with a Delete selected spot action; Find Spots and From dust map… add spots without a pointer and announce the result ("Found 37 dust spots", "No dust spots found", "Dust spots found already have patches"). Touch-up's blemish rings read as "3 blemishes, blemish 2 selected", with a Keep selected spot action; Find Faces announces "Found 2 faces, 1 too small to retouch" and Find Blemishes "Found 12 blemishes", and each face in the panel is a switch, "Face 1", numbered left to right.
- **Models.** In Settings › AI › Models each model reads as one line, "BiRefNet Lite, subject, MIT licence, 103 megabytes, bundled, default", with Use, Get… and Remove as its actions and the Source link naming the model it opens; adding or removing a model announces the result. A mask row whose model is missing reads the sentence saying which model stands in.
...
- **Other windows.** The Loupe on a second display reads the image's name. In the export quality comparison, each pane reads the image and its quality and has Pan left, right, up and down actions, and each Use button says which quality it sets. The slideshow reads each slide's file name and position ("Slideshow, DSC_0107.NEF, 3 of 20"), announces each slide, and announces Slideshow paused and Slideshow playing; its buttons are labelled. The Rename sheet's field says the extension is kept. The HDR Merge dialog reads each photo as one line, as in "DSC_0106.NEF, 1/15 s, ƒ/8, ISO 100, 2 stops brighter than the reference" (with the deghost overlay on, followed by the photo's overlay colour), with a Use as reference action, reads its preview as up to date or updating, and its warnings and errors as such; the merge's progress bar in the left panel reads its stage and percentage, beside a Cancel HDR merge button.
...
- **Announcements.** Errors that appear in the status bar are announced, and so is the end of an export: a batch with how many were exported and how many failed, a single image with its result. So are the end of a move or copy, a finished contact sheet, a finished HDR merge ("HDR merge finished" and the new file's name), and a second display being unplugged. The status bar's progress for moves and copies reads as, for example, "Moving, 3 of 12", beside a Stop button.
...
- Placing a spot patch or brush stroke, placing a red-eye spot, dust spot or blemish by hand, drawing a gradient, radial or brush mask, clicking to select a subject, and moving the crop rectangle need a pointer. Find Spots, Find Faces, Find Blemishes and a Subject mask need none.
