...
/// long as the export queue, Export Open Image, Edit in External Editor, a
/// print, a contact sheet or AI noise reduction is working. Ends when
...
final class ExportActivity: @unchecked Sendable {
    ...
    /// How many are held now, across the app; for tests.
    static var activeCount: Int { countLock.withLock { count } }

    init(reason: String) {
        token = ProcessInfo.processInfo.beginActivity(options: Self.options, reason: reason)
        Self.countLock.withLock { Self.count += 1 }
    }
    ...
    func end() {
        ...
        if let ending {
            ProcessInfo.processInfo.endActivity(ending)
            Self.countLock.withLock { Self.count -= 1 }
        }
    }
